...
public class DialogueVertexAnimator {
    ...
    public Action<float> playAudioFunction;
    public DialogueVertexAnimator(TMP_Text _textBox, Action<string> _setPortraitFunction, Action<float> _playAudioFunction) {
        textBox = _textBox;
    ...
    public IEnumerator AnimateTextIn(List<DialogueCommand> commands, string processedMessage, float _pitchCenter, Action onFinish) {
        textAnimating = true;
            ...
            if (ShouldShowNextCharacter(secondsPerCharacter, timeOfLastCharacter)) {
                if (visableCharacterIndex <= charCount) {
                    ...
                    if (visableCharacterIndex < charCount && ShouldShowNextCharacter(secondsPerCharacter, timeOfLastCharacter)) {
                        ...
                        PlayDialogueSound(_pitchCenter);
    ...
    private void PlayDialogueSound(float _pitchCenter) {
        if (Time.unscaledTime - lastDialogueSound > timeUntilNextDialogueSound) {
            timeUntilNextDialogueSound = UnityEngine.Random.Range(0.05f, 0.12f);
            lastDialogueSound = Time.unscaledTime;
            playAudioFunction?.Invoke(_pitchCenter);
        }
    }
